...
public interface SourceRepository {
	...
	final String c_source = "insert into phsar_source"
							+ "(domain, logo, rows_selector, image_selector, image_attribute, link_selector, title_selector, price_selector, prefix_link, prefix_image) "
							+ "values(#{domain},#{logo}, #{rowsselector}, #{imageselector}, #{imageattribute}, #{linkselector}, #{titleselector}, #{priceselector}, #{prefixlink}, #{prefiximage})";
	
	final String r_source = "select "
							+ "s.source_pk_id, s.domain, s.logo, s.url, s.rows_selector, s.image_selector, s.image_attribute, s.link_selector, s.title_selector, s.price_selector, s.description_selector, s.prefix_link, s.prefix_image "
							+ "from phsar_source s ";
	...
	@Select("SELECT DISTINCT s.* FROM phsar_source s INNER JOIN phsar_source_category sc ON s.source_pk_id = sc.source_fk_id WHERE sc.status = false")
